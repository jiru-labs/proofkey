...
| Brave, built-in model | `Not supported` on Linux and Windows | `availability()` answers `unavailable`: Linux 2026-09-13 (Brave Origin 153), Windows 2026-09-25/26 (Brave 1.96.59 portable, and a user's own Brave on an 8 GB GPU with the reason *The feature flag gating model execution was disabled*). No on-device model in `brave://components`. The Brave row under Providers has the detail |
...
| Brave | Chrome built-in AI | `chrome_builtin` | `Not supported` | Measured 2026-09-13 on Brave Origin 153, on the same laptop Chrome runs it on: `LanguageModel` exists and `availability()` answers `unavailable`. **Windows 11, 2026-09-25/26, the same answer:** portable Brave 1.96.59 (Chromium 154) on a fresh profile, headless over SSH, with and without the features that enable the model in Chrome (`PromptAPIForGeminiNano`, `AIPromptAPI`, `OptimizationGuideOnDeviceModel:BypassPerfRequirement/true`); then a user's own installed Brave in a desktop session, on an AMD Radeon RX 6600 with 8 GB, where the console gives the reason: *The feature flag gating model execution was disabled.* — Blink's text for `kUnavailableFeatureNotEnabled`, a feature switched off in the browser rather than a hardware verdict. `brave://components` lists no on-device model in either. The version of that installed Brave was not read. Up to 0.1.9 the message for that state named Brave only as a possibility beside Chrome's hardware floor, and on 2026-09-15 a Brave user read it as a hardware problem and asked what it meant. On `main`, unreleased, it names Brave outright when `navigator.userAgentData.brands` lists Brave or `navigator.brave` exists — both signals measured 2026-09-15 in Brave 153 on the options page and in the service worker. The new wording is checked by `tools/builtin-check.ts` with stubbed browsers, and on 2026-09-16 was seen in Brave Origin 153.1.95.101 itself: the real service worker returned it to Fix grammar (`test:ext` run in Brave), and the options card and the in-page error toast showed it in full at 1000 px and 360 px wide |
| Any browser without the built-in model | **Find a model on this computer** (options card; on `main`, unreleased) | `chat_completions` | `Tested` | Where `availability()` is `no-api` or `unavailable`, the built-in card offers it. On a click, and never otherwise, it asks for `http://127.0.0.1:1234/*`, `http://localhost:11434/*` and `http://127.0.0.1:8080/*` and reads `/v1/models` on each (LM Studio, Ollama, llama.cpp defaults). `tools/local-servers-check.ts` (in `npm test`) runs it against real loopback servers: a list of models, an empty list, Ollama's 403 read as *running and refusing* with the `OLLAMA_ORIGINS` fix named, a server that never answers (timed out at 300 ms) and a closed port. `test:ext` clicks it in the built extension against a stand-in on port 1234: exactly those three origins are asked for, the server is added and put first, a second click reuses it, and Save keeps it beside the built-in model. **2026-09-26 against a real `llama-server`** (b11120, CPU, `Qwen3-4B-Instruct-2507` Q4_K_M, port 8080) in Playwright's Chromium with the built `dist/`: the click found it with its one model, added it first, and after Save a Fix grammar through the service worker came back corrected (*Todo está bien, pero el informe todavía no está listo.*, 17.5 s on CPU). The permission prompt was stubbed — automation cannot accept it — so that request relied on `llama-server`'s CORS echo, not on granted host access. **Not run against a real LM Studio or Ollama** |
